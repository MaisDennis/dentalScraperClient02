...
export default function Dashboard() {
  const filteredDefaultCremer = cremerData.filter(c => {
    ...
    return titleDetail.toLowerCase().includes('padrão')
  })
  ...
  filteredDefaultCremer.sort(compare);
  filteredDefaultSpeed.sort(compare);
 ...
 function compare (a, b) {
  ...
  const [ cremer, setCremer ] = useState(filteredDefaultCremer);
  const [ cremerListDefault ] = useState(filteredDefaultCremer);
  const [ speed, setSpeed ] = useState(filteredDefaultSpeed);
  const [ speedListDefault ] = useState(filteredDefaultSpeed);
